...
@cf.option_context("future.no_silent_downcasting", True)
def _deprecated_option_context_decorator() -> None:
    pass


def test_option_context_decorator_warns_with_user_stacklevel():
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        line = inspect.currentframe().f_lineno + 1
        _deprecated_option_context_decorator()
